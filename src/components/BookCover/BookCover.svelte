<script>
  ...
  const { id, title, author, cover, variation } = book;
...
{#if interactive}
  <a
    href="/books/{id}"
    use:links
    class="book book--interactive book--variation-{variation} {isValidUrl(cover) ? 'book--cover' : 'book--no-cover'}"
  >
    <span class="cover" style={isValidUrl(book.cover) ? 'background-image: url(' + cover + ')' : ''}>
